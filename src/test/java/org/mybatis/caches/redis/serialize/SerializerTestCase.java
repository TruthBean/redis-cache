...
public class SerializerTestCase {
    ...
    SerializerFactory serializer = new SerializerFactory("kryo");
    ...
    public void testKryoSerialize() {
        ...
        for (int i = 0; i != max; ++i) {
            serializer.serialize(rawSimpleBean);
        }

        byte[] serialBytes = serializer.serialize(rawSimpleBean);
        System.out.println("Byte size after kryo serialize " + serialBytes.length);
        SimpleBean unserializeSimpleBean = (SimpleBean) serializer.deserialize(serialBytes);

        for (int i = 0; i != max; ++i) {
            serializer.deserialize(serialBytes);
        }
